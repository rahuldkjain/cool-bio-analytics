...
export const abbreviateNumber = (number) => {
  ...
  else if (Math.abs(number) >= 1e3 && Math.abs(number) < 1e5) { return numberFormatter.format(number / 1e3) + 'K' } else if (Math.abs(number) >= 1e5 && Math.abs(number) < 1e7) { return numberFormatter.format(number / 1e5) + 'L' } else if (Math.abs(number) >= 1e7 && Math.abs(number) < 1e10) { return numberFormatter.format(number / 1e7) + 'Cr' } else if (Math.abs(number) >= 1e10 && Math.abs(number) < 1e14) { return numberFormatter.format(number / 1e10) + 'K Cr' } else if (Math.abs(number) >= 1e14) { return numberFormatter.format(number / 1e14) + 'L Cr' }
}
...
function calculateBouce (session = 0, user = 0) {
  const bounce = 100 - (user / session) * 100
  return isNaN(bounce) ? 0 : bounce
}

export function getStatisticData (data, statistic) {
  switch (statistic) {
    case 'users' :
      return {
    ...
    case 'active' :
      return {
        ...
        delta: data?.active?.aggregate?.delta || 0
      }
