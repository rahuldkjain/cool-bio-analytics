...
function Map ({ mapStatistic, projectId }) {
  ...
  const { loading, error, data = {} } = useQuery(queries[mapStatistic], {
    variables: {
      projectId,
      at: {
  ...
  const handleGeographyClick = (geography) => event => {
    const currentSelectedData = table.find(item => item.country === (geography.properties['Alpha-2']))
    setSelectedCountry({
        ...
        {({ geographies }) =>
          geographies.map(geo => {
            return <Geography
              ...
              style={{
                default: {
                  fill: highligts.includes(geo.properties['Alpha-2']) ? color : `${color}10`,
                  stroke: highligts.includes(geo.properties['Alpha-2']) ? color : `${color}50`,
                  strokeWidth: 2,
                  outline: 'none'
                },
                hover: {
