...
function CustomTooltip ({ statistic, item, payload = [], defaultData }) {
  return (
        <Stats>
            ...
            <Title statistic={statistic}>{payload?.length && payload[0]?.payload ? payload[0]?.payload?.name : 'Today'}</Title>
            <StatsBottom statistic={statistic}>
                <h2>
                    {formatNumber(
                      payload && payload?.length && payload[0] ? payload[0].value : getStatisticData(defaultData, statistic).count,
                      item.format !== 'short'
                        ? item.format
                        : 'int',
                      statistic
